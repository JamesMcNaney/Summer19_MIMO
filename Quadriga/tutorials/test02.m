...
a_28000_MHz(1) = qd_arrayant( '3gpp-3d',  8, 8, s.center_frequency(2), 3 );
a_28000_MHz(2) = qd_arrayant( '3gpp-3d',  8, 8, s.center_frequency(2), 3 );


l.tx_array(1,1) = a_2600_Mhz;                           % Set 2.6 GHz antenna
l.tx_array(2,1) = a_28000_MHz(1);                          % Set 28 Ghz antenna
l.tx_array(3,1) = a_28000_MHz(2);                          % Set 28 Ghz antenna
...
P_db1 = 10*log10( sum( map{1}, 4 ) );

% Plot the results
l.visualize([],[],0);                                   % Show BS and MT positions on the map
hold on; imagesc( x_coords, y_coords, P_db1 ); hold off  % Plot the antenna footprint
axis([x_min,x_max,y_min,y_max]);
caxis( max(P_db1(:)) + [-20 0] );                        % Color range
colmap = colormap;
...
beam_x1 = find( x_coords >= 200 , 1 );                   % Point the beam at x = 200 and y = 100
beam_y1 = find( y_coords >= -100  , 1 );
w1 = conj( map{1}( beam_y1, beam_x1 , 1 ,: ) );            % Precoding weights for a MRT beamformer
w1 = w1 ./ sqrt(mean(abs(w1(:)).^2));                      % Normalize to unit power

beam_x2 = find( x_coords >= 200 , 1 );                   % Point the beam at x = 200 and y = 100
beam_y2 = find( y_coords >= 100  , 1 );
w2 = conj( map{1}( beam_y2, beam_x2 , 1 ,: ) );            % Precoding weights for a MRT beamformer
w2 = w2 ./ sqrt(mean(abs(w2(:)).^2));                      % Normalize to unit power

% Apply the precoding weights to each pixel on the map and calculate the received power
P_db1 = map{1} .* w1( ones(1,numel(y_coords)), ones(1,numel(x_coords)),:,: );
P_db1 = 10*log10( abs( sum( P_db1 ,4 ) ).^2 );

P_db2 = map{1} .* w2( ones(1,numel(y_coords)), ones(1,numel(x_coords)),:,: );
P_db2 = 10*log10( abs( sum( P_db2 ,4 ) ).^2 );

l.visualize([],[],0);                                   % Show BS and MT positions on the map
Pdb_sum = P_db1 + P_db2;
hold on; imagesc( x_coords, y_coords, Pdb_sum );
%imagesc( x_coords, y_coords, P_db2 ); 

hold off  % Plot the antenna footprint
axis([x_min,x_max,y_min,y_max]);
%caxis( max(P_db1(:)) + [-100 0] );                        % Color range
caxis( max(P_db2(:)) + [-100 -25] );                        % Color range
colmap = colormap;
colormap( colmap*0.5 + 0.5 );                           % Adjust colors to be "lighter"
set(gca,'layer','top')                                  % Show grid on top of the map
colorbar('south')
