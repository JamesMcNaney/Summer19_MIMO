...
l.track(1,1) = qd_track('circular',20*pi,0);            % Circular track with 10m radius
l.track(1,1).initial_position  = [10;0;1.5];            % Start east, running north
l.track(1,1).segment_index     = [1,40,90];             % Segments
l.track(1,1).scenario          = { UMal, UMan, UMal };  % Scenarios

l.track(1,2) = qd_track('linear',20,pi/8);              % Linear track, 20 m length
l.track(1,2).initial_position  = [10;0;1.5];            % Same start point
l.track(1,2).interpolate_positions( 128/20 );
l.track(1,2).segment_index     = [1,40,90];             % Segments
l.track(1,2).scenario          = { UMal, UMal, UMan };  % Scenarios

set(0,'DefaultFigurePaperSize',[14.5 7.3])              % Adjust paper size for plot
l.visualize;                                            % Plot the layout

interpolate_positions( l.track, s.samples_per_meter );  % Interpolate
...
tic; c = get_channels( p ); cn = merge( c ); toc;       % Generate channel coefficients

disp('Drifting disabled:');
s.use_spherical_waves = 0;                              % Disable drifting
tic; d = get_channels( p ); dn = merge( d ); toc;       % Generate channel coefficients
