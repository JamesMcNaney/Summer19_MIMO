...
for i = 1:8
    for j = 1:16
        ...
        a(i,j) = qd_arrayant('multi', 8, 0.5, 12 );                  % Generate High-Gain Antenna
        l.tx_position(:,k) = [-50 ; half_lamb*j ; 5 + half_lamb*i];
        l.tx_array(1,k) = a(i,j);
        k = k + 1;
...
a = qd_arrayant('3gpp-3d',8,16,300000000,1,12);                  % Generate High-Gain Antenna
...
[ map,x_coords,y_coords] = l.power_map( '3GPP_38.901_UMa_LOS','quick',5,100,0,-5,55,1.5 );
P = 10*log10(sum( abs( cat(3,map{:}) ).^2 ,3));         % Total received power
